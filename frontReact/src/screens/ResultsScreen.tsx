          ...
          {filtered.length === 0 ? (
            ...
            <>
              ...
              {filtered.map((course, index) => (
                <FadeIn key={`${course.name}-${index}`} delay={200 + index * 50} fromX={24}>
...
const StatsBar: React.FC<{ total: number; tier1: number; dept: string; accent: string }> = ({ total, tier1, dept, accent }) => (
...
const TierFilterRow: React.FC<{ selected: string; accent: string; counts: Record<string, number>; onSelect: (t: string) => void }> = ({ selected, counts, onSelect }) => {
      ...
      {tiers.map((t) => {
        ...
        return (
          <Pressable key={t} onPress={() => onSelect(t)}>
            <View style={[styles.filterChip, { backgroundColor: sel ? addAlpha(color, 0.15) : AppColors.surfaceElevated, borderColor: sel ? color : AppColors.border, borderWidth: sel ? 1.5 : 1 }]}>
              ...
              <View style={[styles.filterChipBadge, { backgroundColor: sel ? addAlpha(color, 0.2) : AppColors.border }]}>
                <Text style={[styles.filterChipBadgeText, { color: sel ? color : AppColors.textMuted }]}>{counts[t] || 0}</Text>
              </View>
            </View>
          </Pressable>
        );
    ...
    <View style={styles.cardWrapper}>
      <GlowCard glowColor={tierColor} isSelected={isTop} padding={16}>
        <View style={styles.cardHeader}>
          <View style={[styles.cardRankBox, { backgroundColor: rank <= 3 ? addAlpha(tierColor, 0.15) : AppColors.surface, borderColor: rank <= 3 ? tierColor : AppColors.border, borderWidth: isTop ? 2 : 1 }]}>
            <Text style={[styles.cardRankText, { color: rank <= 3 ? tierColor : AppColors.textMuted }]}>#{rank}</Text>
          </View>
...
const MetaChip: React.FC<{ icon: keyof typeof Ionicons.glyphMap; label: string; color: string }> = ({ icon, label, color }) => (
...
const styles = StyleSheet.create({
  ...
  glowBlobContainer: { position: 'absolute', top: -80, right: -80, width: 300, height: 300, zIndex: 0 },
  safeArea: { flex: 1 },
  appBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingTop: 12, zIndex: 10 },
  backButton: { padding: 8 },
  mockBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: AppColors.amberSoft, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, borderWidth: 1, borderColor: addAlpha(AppColors.amber, 0.4) },
  mockBadgeText: { fontSize: 11, color: AppColors.amber, fontWeight: '600' },
  scrollContent: { paddingHorizontal: 24, paddingTop: 16 },
  headerRow: { flexDirection: 'row', alignItems: 'flex-start' },
  headerIconBox: { width: 44, height: 44, borderRadius: 12, borderWidth: 1, justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  headerTextCol: { flex: 1 },
  headerTitle: { fontFamily: 'Syne-ExtraBold', fontSize: 22, fontWeight: '800', color: AppColors.textPrimary, letterSpacing: -0.5 },
  headerSubtitle: { fontSize: 11, fontWeight: '600', marginTop: 2 },
  statsContainer: { flexDirection: 'row', backgroundColor: AppColors.surface, borderRadius: 14, borderWidth: 1, borderColor: AppColors.border, padding: 16 },
  statsDivider: { width: 1, height: 30, backgroundColor: AppColors.border, alignSelf: 'center' },
  statCol: { flex: 1, alignItems: 'center' },
  statValue: { fontFamily: 'Syne-ExtraBold', fontSize: 15, fontWeight: '800', marginBottom: 2 },
  statLabel: { fontSize: 10, color: AppColors.textMuted, fontWeight: '600' },
  filterScroll: { overflow: 'visible' },
  filterChip: { flexDirection: 'row', alignItems: 'center', marginRight: 8, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20 },
  filterChipText: { fontSize: 12, fontWeight: '700', marginRight: 5 },
  filterChipBadge: { paddingHorizontal: 5, paddingVertical: 1, borderRadius: 8 },
  ...
  listCountText: { fontSize: 11, fontWeight: '600' },
  cardWrapper: { marginBottom: 12 },
  cardHeader: { flexDirection: 'row', alignItems: 'flex-start' },
  cardRankBox: { width: 34, height: 34, borderRadius: 17, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  cardRankText: { fontSize: 10, fontWeight: '800' },
  cardTitleCol: { flex: 1, marginRight: 8 },
  cardTitle: { fontFamily: 'Syne-Bold', fontSize: 14, fontWeight: '700', color: AppColors.textPrimary, lineHeight: 18, marginBottom: 4 },
  cardDepartment: { fontSize: 11, fontWeight: '600' },
  cardDivider: { height: 1, backgroundColor: addAlpha(AppColors.border, 0.5), marginVertical: 12 },
  metaChipsContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  cardFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 },
  tierLabelBox: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, borderWidth: 1 },
  tierLabelText: { fontSize: 10, fontWeight: '700', letterSpacing: 0.3 },
  topPickBox: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  topPickText: { fontSize: 9, fontWeight: '800', letterSpacing: 1 },
  matchBadgeBox: { width: 52, height: 52, borderRadius: 26, borderWidth: 1.5, justifyContent: 'center', alignItems: 'center' },
  matchBadgeValue: { fontFamily: 'Syne-ExtraBold', fontSize: 15, fontWeight: '800', lineHeight: 15 },
  matchBadgePercent: { fontSize: 9, fontWeight: '600', marginTop: -2 },
  metaChipBox: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, borderWidth: 1 },
  metaChipText: { fontSize: 11, fontWeight: '600' },
  emptyContainer: { alignItems: 'center', paddingVertical: 60 },
  emptyTitle: { fontFamily: 'Syne-Bold', fontSize: 20, fontWeight: '700', color: AppColors.textPrimary, marginTop: 16, marginBottom: 8 },
  emptySubtitle: { fontSize: 13, color: AppColors.textSecondary, lineHeight: 19.5, textAlign: 'center' },
