...
class StudentForm {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'years': years.map((y) => y.toString()).toList(),
...
class CourseResult {
  ...
  final String difficulty;
  final String? prerequisite;   // null when backend sends NaN / null
  ...
  /// Parse one item from the backend `data` array.
  factory CourseResult.fromJson(Map<String, dynamic> json) {
    final details = json['details'] as Map<String, dynamic>? ?? {};
    final rawPrereq = details['prerequisite'];
    return CourseResult(
      ...
      prerequisite:    (rawPrereq == null || rawPrereq.toString() == 'null' || rawPrereq.toString() == 'NaN')
                           ? null
                           : rawPrereq.toString(),
      preference:      details['preference']?.toString() ?? '',
      yearOfStudy:     (details['year_of_study'] as num?)?.toInt() ?? 0,
      department:      details['department']?.toString() ?? '',
    );
  }

  // ── Mock data keeps the same shape for offline demo ──────────
  static List<CourseResult> mockResults(String dept) {
    final raw = [
      ...
      },
      {
        ...
        'match_tier': 'Tier 2 (75%)',
        'details': {'difficulty': 'Hard', 'prerequisite': 'Intro to CSE', 'preference': 'Programming', 'year_of_study': 4, 'department': dept},
      },
      {
        'course_name': 'Introduction to Cybersecurity',
        'match_percentage': 50.0,
        'match_tier': 'Tier 3 (50%)',
        'details': {'difficulty': 'Medium', 'prerequisite': 'Intro to CSE', 'preference': 'Math', 'year_of_study': 4, 'department': dept},
      },
      {
        'course_name': 'Fundamentals of Web Development',
        'match_percentage': 25.0,
        'match_tier': 'Low Match',
        'details': {'difficulty': 'Medium', 'prerequisite': 'Intro to CSE', 'preference': 'Management', 'year_of_study': 5, 'department': dept},
      },
    ];
    return raw.map((e) => CourseResult.fromJson(e)).toList();
  }
}
...
class AppConstants {
  ...
  static const List<String> allCourses = [
    // Architecture
